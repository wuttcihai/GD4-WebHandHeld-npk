...
export class ConveyorService {
  ...
  getConveyorSetting(): Observable<DeviceResponse> {
    return this.http.get<DeviceResponse>(`${this.apiUrl}/setting/conveyor/all`).pipe(
      shareReplay(1),
  ...
  updateConveyorSetting(resultData: any): Observable<DeviceResponse> {
    return this.http.post<DeviceResponse>(`${this.apiUrl}/setting/conveyor/update`, resultData).pipe(
      tap((response: { status: number; }) => {
        if (response.status === 200) {
          // this.toastr.success('Successful!', 'แจ้งเตือน');
        }
